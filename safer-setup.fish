...
function main -a source_disk -a target_disk -d "Eternal-ify from source->target disk"
    echo $source_disk $target_disk

    string length -q $source_disk; or panic "must specify a source disk!"
    string length -q $target_disk; or panic panic "must specify a target disk!"
